...
from s3 import get_client3 as get_client
from app_config import (
    CONFIG_PATH as APP_CONFIG_PATH,
    clear_session as clear_app_session,
    load_app_settings,
    load_valid_session_username,
    save_app_settings,
    save_session as save_app_session,
    set_ui_dark_preference,
)
...
_initial_settings = load_app_settings()
THEME_PALETTE = {}
INPUT_MAX_WIDTH = 200
...
class RoundedButton(tk.Canvas):
    def __init__(self, master, text, command=None, primary=True, radius=10, pad_x=18, pad_y=10):
        ...
        if not bg_color or bg_color in ("", "SystemButtonFace"):
            bg_color = THEME_PALETTE.get("SURFACE", "#1b1d22")
    ...
    def _redraw(self, event=None):
        ...
        base, _, textcol = self._colors()
        self._round_rect(1, 1, w-1, h-1, self.radius, fill=base, outline="", width=1)
...
def _should_skip_rounded_upgrade(widget):
    try:
        ...
        while isinstance(w, tk.Misc) and w is not None:
            if isinstance(w, LoginFrame):
                return True
            # Main app tabs are relaid out dynamically and depend on stable Entry widgets.
...
def _replace_entry_with_rounded(entry):
    ...
    """
    try:
        if _should_skip_rounded_upgrade(entry):
            return
...
def _upgrade_inputs_in(widget):
    ...
    try:
        for child in list(widget.winfo_children()):
            # Skip areas whose layouts depend on the original Entry widgets.
            if isinstance(child, LoginFrame) or isinstance(child, ttk.Notebook):
                continue
            # Recurse first
...
class LoginFrame(ttk.Frame):
    def __init__(self, master, on_success, on_cancel):
        ...
        try:
            ...
            self.primary_btn.configure(width=20)
        except Exception:
            pass
    ...
    def _toggle_theme(self):
        ...
        try:
            cfg = set_ui_dark_preference(new_dark, settings=load_app_settings())
    ...
    def _finalize(self, user_id, username):
        ...
        try:
            ...
            save_app_session(username, bool(keep is not None and keep.get()))
        except Exception:
            pass
...
def create_topbar(parent, username, on_logout):
    ...
    lbl_title = ttk.Label(
        inner,
        text="S3 GoSimply Manager",
        style="TopbarTitle.TLabel",
        anchor="w",
        wraplength=0 
    )
    lbl_title.grid(row=0, column=0, sticky="w", padx=(16, 12), pady=(8, 8))

    # Right side: user avatar + name + logout
    right = ttk.Frame(inner, style="TopbarInner.TFrame")
    right.grid(row=0, column=1, sticky="e", padx=(12, 12), pady=(6, 6))
    ...
    avatar.grid(row=0, column=0, sticky="w", padx=(0, 6))
    ...
    lbl_username.grid(row=0, column=1, sticky="w", padx=(0, 10))
    # Expose for updates
    ...
    logout_btn.grid(row=0, column=2, sticky="e", padx=(6, 0), pady=0)
    ...
    def _relayout_topbar(event=None):
        if event is not None and getattr(event, "widget", None) is not topbar:
            return
        try:
            width = max(topbar.winfo_width(), inner.winfo_width(), 1)
        except Exception:
            width = 1
        compact = width < 860
        very_compact = width < 680
        state = (compact, very_compact)
        if getattr(topbar, "_layout_state", None) == state:
            return
        topbar._layout_state = state
        try:
            if compact:
                inner.grid_columnconfigure(1, weight=1)
                lbl_title.grid_configure(row=0, column=0, columnspan=2, sticky="w", padx=(16, 12), pady=(8, 2))
                right.grid_configure(row=1, column=0, columnspan=2, sticky="ew", padx=(16, 12), pady=(0, 8))
            else:
                inner.grid_columnconfigure(1, weight=0)
                lbl_title.grid_configure(row=0, column=0, columnspan=1, sticky="w", padx=(16, 12), pady=(8, 8))
                right.grid_configure(row=0, column=1, columnspan=1, sticky="e", padx=(12, 12), pady=(6, 6))
        except Exception:
            pass
        try:
            lbl_title.configure(text=("S3 GoSimply" if very_compact else "S3 GoSimply Manager"))
            lbl_username.grid_remove() if very_compact else lbl_username.grid()
            logout_btn.configure(width=(74 if very_compact else 90))
        except Exception:
            pass
    ...
    topbar.bind("<Configure>", _relayout_topbar)
    topbar.after(0, _relayout_topbar)

    # Keep references for future theme changes if needed
    topbar._gradient_canvas = canvas
    topbar._overlay = inner
    topbar._title_label = lbl_title
    topbar._right_frame = right
    return topbar
...
def _start_login(root, card_factory):
    ...
    try:
        username = load_valid_session_username()
        if username:
...
def show_login_modal(parent, card_factory):
    ...
    container.pack(expand=True, padx=(60, 80), pady=60)
    container.configure(width=900, height=640)
...
def apply_theme(root):
    ...
    style.configure("PrimaryLarge.TButton", padding=(22,16), font=pick_font(DISPLAY_STACK, 14, "bold"),
                    background=accent_idle, foreground="white", borderwidth=0)
    # Keep pressed/active states in accent colors to avoid white flash on Windows
    style.map(
    ...
    style.configure("GhostSmall.TButton", padding=(12,8), font=FONT_TEXT,
                    background=ghost_bg, foreground=TEXT, bordercolor=_blend_hex(ghost_bg, BG, 0.35), borderwidth=1)
...
def _display_config_path() -> str:
    try:
        return "~/" + str(APP_CONFIG_PATH.relative_to(Path.home()))
    except Exception:
...
ls_metric_count.grid(row=0, column=0, sticky="w")
ls_metric_size.grid(row=0, column=1, sticky="w", padx=(12, 0))
ls_metric_prefix.grid(row=0, column=2, sticky="w", padx=(12, 0))
...
settings_shell = ttk.Frame(settings_tab, style="Section.TFrame")
settings_shell.pack(fill="both", expand=True)
settings_shell.grid_columnconfigure(0, weight=1)
settings_shell.grid_rowconfigure(0, weight=1)
...
s_card = card(settings_canvas)
settings_window = settings_canvas.create_window((0, 0), window=s_card, anchor="nw")
...
def _resize_settings_canvas(event):
    try:
        settings_canvas.itemconfigure(settings_window, width=max(event.width, 1))
    except Exception:
        pass
...
def _bind_settings_mousewheel(_event=None):
    ...
    try:
        ...
        settings_canvas.bind_all("<Button-4>", _on_mousewheel)
        settings_canvas.bind_all("<Button-5>", _on_mousewheel)
    except Exception:
        pass
...
settings_canvas.bind("<Configure>", _resize_settings_canvas)
s_card.bind("<Configure>", _update_settings_scrollregion)
settings_canvas.bind("<Enter>", _bind_settings_mousewheel)
settings_canvas.bind("<Leave>", _unbind_settings_mousewheel)

TAB_LABELS = [
    (upload_tab, "📤 Upload", "📤 Up"),
    (dl_tab, "📥 Download", "📥 Down"),
    (ls_tab, "📄 List", "📄 List"),
    (delobj_tab, "🗑️ Delete Object", "🗑️ Object"),
    (delbucket_tab, "🗑️ Delete Bucket", "🗑️ Bucket"),
...
s_callout_text = ttk.Label(
    s_callout,
    text="Store your S3 GoSimply credentials locally so uploads and downloads work without environment variables.",
    style="AccentCalloutText.TLabel",
    justify="left"
)
...
s_provider_aws.grid(row=0, column=0, sticky="w", padx=(0,12))
s_provider_minio.grid(row=0, column=1, sticky="w")
...
path_style_switch.grid(row=0, column=0, sticky="w", padx=(0, 8))
path_style_hint.grid(row=0, column=1, sticky="we")
...
https_switch.grid(row=0, column=0, sticky="w", padx=(0, 8))
https_hint.grid(row=0, column=1, sticky="we")
...
s_test_status_label.grid(row=0, column=0, sticky="w")
s_btn_save.grid(row=0, column=1, sticky="e", padx=(8,0), pady=(0,2))
s_btn_test.grid(row=0, column=2, sticky="e", padx=(8,0), pady=(0,2))
...
s_info_section = ttk.Frame(settings_shell, style="Info.TFrame", padding=(22,18))
...
s_info_section.grid(row=1, column=0, columnspan=2, sticky="ew", padx=12, pady=(0, PADY))
...
def _on_provider_change(*_):
    ...
    if provider == PROVIDER_AWS:
        ...
        s_lbl_endpoint.config(text="S3 Endpoint")
        s_lbl_access.config(text="Access Key ID")
        s_access_hint.config(text="Example: AKIAIOSFODNN7EXAMPLE", style="SectionHint.TLabel")
        s_lbl_secret.config(text="Secret Access Key")
        s_secret_hint.config(text="Paste the secret key exactly as provided.", style="SectionHint.TLabel")
        cfg_secure.set(True)
    else:
        s_chk_custom_endpoint.state(["!disabled"])
        s_lbl_region.config(text="Region (optional)")
        s_region_hint.config(text="Example: us-east-1 (leave blank to use server default)")
        s_lbl_endpoint.config(text="Server Endpoint")
        s_access_hint.config(text="Example: minioadmin", style="SectionHint.TLabel")
        s_lbl_access.config(text="Access Key")
        s_lbl_secret.config(text="Secret Key")
        s_secret_hint.config(text="Example: minioadmin or the secret key from your MinIO server.", style="SectionHint.TLabel")
        if not restored:
            cfg_region.set("")
            cfg_access_key.set("")
            cfg_secret_key.set("")
...
def _set_test_status(message, style="StatusInfo.TLabel"):
    ...
    if message:
        s_test_status_label.grid_configure(padx=(0,8))
    else:
        s_test_status_label.grid_configure(padx=(0,0))
...
def _validate_fields(*_):
    ...
    if provider == PROVIDER_AWS:
        ...
        if valid_access:
            ...
            s_access_hint.config(text="🔴 Access key is required for MinIO", style="Error.TLabel")
...
def _schedule_wrap_refresh(delay=0):
    try:
        existing = getattr(root, "_wrap_refresh_after", None)
        if existing:
            root.after_cancel(existing)
    except Exception:
        pass
    try:
        root._wrap_refresh_after = root.after(delay, _update_progress_wrap)
    except Exception:
        pass
            ...
            if effective <= 24:
                try:
                    ...
                    if parent is not None:
                        parent_width = max(int(parent.winfo_width()), int(parent.winfo_reqwidth()))
                        if parent_width > 24:
                            effective = parent_width
                except Exception:
                    pass
            if effective <= 24:
                try:
                    parent = lbl.master
                    grand = parent.master if parent is not None else None
                    if grand is not None:
                        grand_width = max(int(grand.winfo_width()), int(grand.winfo_reqwidth()))
                        if grand_width > 24:
                            effective = grand_width
                except Exception:
                    pass
            if effective <= 24:
                effective = wrap
            candidate = effective - 40 if effective > 160 else effective
            lbl.configure(wraplength=max(100, min(wrap, candidate)))
        except Exception:
            pass

def _relayout_flow_rows():
    def flow(frame, widgets, min_width=240, row_gap=8, col_gap=12):
        try:
            width = max(frame.winfo_width(), frame.winfo_reqwidth(), 1)
        except Exception:
            width = 1
        count = len(widgets)
        if count <= 0:
            return
        cols = max(1, min(count, width // max(min_width, 1)))
        if getattr(frame, "_flow_cols", None) == cols:
            return
        frame._flow_cols = cols
        current_cols = max(count, cols)
        ...
        for index, widget in enumerate(widgets):
            ...
            try:
                widget.grid_configure(
                    ...
                    column=col,
                    sticky="we",
                    padx=(0 if col == 0 else col_gap, 0),
    ...
    flow(s_provider_opts, [s_provider_aws, s_provider_minio], min_width=180)
    flow(path_style_row, [path_style_switch, path_style_hint], min_width=280)
    flow(https_row, [https_switch, https_hint], min_width=280)
    flow(s_actions, [s_test_status_label, s_btn_save, s_btn_test], min_width=220)
    flow(ls_metrics, [ls_metric_count, ls_metric_size, ls_metric_prefix], min_width=220)

def _update_tab_presentation():
    try:
        width = max(root.winfo_width(), 1)
    except Exception:
        width = 1
    compact = width < 1120
    if getattr(notebook, "_tab_compact", None) == compact:
        return
    notebook._tab_compact = compact
    try:
        style = ttk.Style(root)
        style.configure("TNotebook.Tab", padding=((10, 7) if compact else (14, 8)), font=FONT_TEXT, background=THEME_PALETTE.get("BG", "#141518"))
    except Exception:
        pass
    for tab_widget, full_text, compact_text in TAB_LABELS:
        try:
            notebook.tab(tab_widget, text=(compact_text if compact else full_text))
        except Exception:
            pass
...
root.after(120, _update_progress_wrap)
root.after(0, _relayout_flow_rows)
root.after(0, _update_tab_presentation)
